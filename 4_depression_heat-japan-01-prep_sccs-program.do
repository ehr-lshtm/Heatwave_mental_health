/*=========================================================================
DO FILE NAME:			4_depression_heat-japan-01-prep_sccs-program
...
log using "${pathLogs}/4_depression_heat-japan-01-prep_sccs-program", text replace
*********************************************************************
...
forval num = 1/6 {
	...
	save "depression_clean_drug_all_`num'", replace
	restore
}
...
use "depression_clean_drug_all_`num'", clear
...
save "depression_clean_drug_all_heatwave_`num'", replace

}

*combine all dataset into one
use "depression_clean_drug_all_heatwave_1", clear
forval num = 2/6 {
append using "depression_clean_drug_all_heatwave_`num'"
}
save "$datafile/depression_clean_drug_all_heatwave", replace
...
merge 1:m patid interstart using "$datafile/depression_clean_drug_all_heatwave", keepusing(exp_st exp_en) nogen

save "$datafile/depression_drug_heatwave_cat", replace
	
/*****************************************************************
* 3. Create heat exposure intervals for each drug exposed interval
*****************************************************************/	
forval num_interval = 1/`max' {
use "$datafile/depression_drug_heatwave_cat", clear
...
save "$datafile/depression_drug_intervals", replace
