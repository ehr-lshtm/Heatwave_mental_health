/*=========================================================================
DO FILE NAME:			3_psychiatric_heat-japan-01-set-up-SCCS
...
global filename "3_psychiatric_heat-japan-01-set-up-SCCS"
...
**************************************/
use "$datafile/smd_dxall.dta", clear
rename 加入者id patid
sort patid eventdate
bysort patid: keep if _n==1
unique patid
save "$datafile/psychiatric_all", replace
...
forval city_num = 1/6 {
        ...
        save "psychiatric_`city_num'", replace
        restore
}
...
use "psychiatric_`city_num'", clear
...
save "$datafile/psychiatric_heatwave_`city_num'", replace
}

*combine all dataset into one
use "$datafile/psychiatric_heatwave_1", clear
forval city_num = 2/6 {
append using "$datafile/psychiatric_heatwave_`city_num'"
}
save "$datafile/psychiatric_heatwave", replace


*erase unneccessary datasets 
forval city_num = 1/6 {
erase "$datafile/psychiatric_heatwave_`city_num'.dta"
}
...
keep if substr(who_atc, 1, 4) == "N05A"
tab who_atc,m //check with Chitose the drugs that have no details of atc
*only N05A, N05AD, N05AL, N05AX

replace who_atc = "N05AA01" if who_atc == "N05A" //combination of chlorpromazine + phenobarbital + promethazine
replace who_atc = "N05AD01" if who_atc == "N05AD" //both ingredients are haloperidol
//for the rest including N05AL and N05AX are not classified as they are Japanese drugs
...
save "$datafile/psychotropic_drug_N05A", replace

*save a record only have one patient per row
duplicates drop 加入者id, force
save "$datafile/psychotropic_drug_N05A_hc", replace
...
save "psychotropic_drug_N05A_rd", replace
restore
...
merge m:1 加入者id using "$datafile/psychotropic_drug_N05A_hc", ///
...
save "$datafile/psychiatric_clean_drug_all", replace
...
use "$datafile/psychiatric_heatwave", clear
merge m:1 patid using `psychiatric_drug', keep(master) nogen
save "$datafile/psychiatric_heatwave_noantipsy", replace
