/*=========================================================================
DO FILE NAME:			8_antipsy_heat-japan-01-stratify-antipsy-set-up-SCCS
...
log using "${pathLogs}/8_antipsy_heat-japan-01-stratify-antipsy-set-up-SCCS", text replace

/*************************************
*Antipsychotic users (take treatment discontinuation into account)
**************************************/
use "$datafile/psychotropic_drug_N05A", clear
unique 加入者id
...
*Remove anyone who had antipsychotics 180 days before the first prescription
merge 1:m 加入者id using "$datafile/psychotropic_drug_N05A", keepusing(rxst rxen) keep(master match) nogen
...
*flag those who had multiple antipsychotics overlapped the start date
merge 1:m 加入者id using "psychotropic_drug_N05A_rd", keepusing(rxst drug_type) keep(master match) nogen
...
merge 1:1 加入者id using "$datafile/psychotropic_drug_N05A_hc", ///
...
su st_st, format
gen st_st_new = max(st_st, ob_st_date, eventdate)
...
save "$datafile/antipsy_first_clean_drug_all", replace

*key city category code to city code
use "$datafile/antipsy_first_clean_drug_all", clear
...
forval num = 1/6 {
	...
	save "antipsy_first_clean_drug_all`num'", replace
	restore
}
...
use "antipsy_first_clean_drug_all`num'", clear
...
save "antipsy_clean_drug_all_heatwave_`num'", replace

}

*combine all dataset into one
use "antipsy_clean_drug_all_heatwave_1", clear
forval num = 2/6 {
append using "antipsy_clean_drug_all_heatwave_`num'"
}
save "$datafile/antipsy_clean_drug_all_heatwave", replace

*erase unneccessary datasets 
forval num = 1/6 {
erase "antipsy_clean_drug_all_heatwave_`num'.dta"
erase "antipsy_first_clean_drug_all`num'.dta"
}

*stratify into 5 commonly used antipsychotics
use "$datafile/antipsy_clean_drug_all_heatwave", clear
duplicates drop patid, force
rename patid 加入者id
drop rxst drug_type
merge 1:m 加入者id using "$datafile/psychotropic_drug_N05A", keep(master match) keepusing(who_atc drug_type rxst rxen) nogen
drop if rxen<st_st
drop if rxst>st_en
...
label var cat_num_psy "Type of antipsychotics in the observation period"
label def psy_cat 0 "Only one antipsychotic" 1 "More than one antipsychotics" 
...
gen drug_interest = 1 if drug_type == 28
replace drug_interest = 2 if drug_type == 25
replace drug_interest = 3 if drug_type == 16
replace drug_interest = 4 if drug_type == 17
replace drug_interest = 5 if drug_type == 20
replace drug_interest = 6 if drug_interest == .
replace drug_interest = 6 if cat_num_psy == 1
label var drug_interest "Type of antipsychotics of interest in the observation period"
label def drug_int_cat 1 "aripiprazole" 2 "risperidone" 3 "olanzapine" ///
 4 "Quetiapine" 5 "sulpride"  6 "Others"
label val drug_interest drug_int_cat
rename 加入者id patid
keep patid cat_num_psy drug_interest
save "$datafile/antipsy_clean_drug_all_heatwave_drugcat", replace
